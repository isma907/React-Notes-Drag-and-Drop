...
import type { StickyNotePosition } from "../interfaces/StickyNote";
import {
  MIN_DRAG_CREATION_WIDTH,
  MIN_DRAG_CREATION_HEIGHT,
...
export function useDragCreate(
  ...
  const onDragCreate = useCallback(
    (e: React.PointerEvent) => {
      if (!dragGuideLinesRef.current || !boardRef.current || e.button !== 0)
        return;
  ...
  const onHoldDragCreate = useCallback(
    (e: React.PointerEvent) => {
  ...
  const onDropCreate = useCallback(
    (e: React.PointerEvent) => {
      if (
        !isDraggingCreation.current ||
        !boardRef.current ||
        !dragGuideLinesRef.current
      )
        return;
